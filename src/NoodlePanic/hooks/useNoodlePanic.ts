...
import { FIELD_H, FIELD_W, type EndReason, type Phase } from '../types'
...
export function useNoodlePanic() {
  ...
  const scoreRef = useRef(0), comboRef = useRef(0), phaseRef = useRef<Phase>('start'), strainRef = useRef(strain), splittingRef = useRef(false)
  ...
  const finish = useCallback((why: EndReason) => {
    ...
    if (why === 'time') { setNextCycle(current => current + 1); setNextStrain(generateNextStrain(strainRef.current)) }
    if (scoreRef.current > best) { setBest(scoreRef.current); localStorage.setItem('noodle-panic.best', String(scoreRef.current)) }
    tone(why === 'noodle' ? 150 : 880, why === 'noodle' ? .26 : .11, why === 'noodle' ? 'sawtooth' : 'triangle', .055)
  }, [best, tone])
  ...
  const start = useCallback(() => { scoreRef.current = 0; comboRef.current = 0; phaseRef.current = 'playing'; strainRef.current = nextStrain; turns.current = 0; splittingRef.current = false; setRunId(value => value + 1); setCycle(nextCycle); setStrain(nextStrain); setIsSplitting(false); setScore(0); setCombo(0); setLit([]); setHead(5); setBoil(false); setSeconds(20); setPhase('playing'); started.current = performance.now(); lastMove.current = started.current; raf.current = requestAnimationFrame(loop) }, [loop, nextCycle, nextStrain])
  const tap = useCallback((cell: number) => {
    ...
    setLit(old => old.includes(cell) ? old : [...old, cell])
    if (lit.includes(cell)) { comboRef.current = 0; setCombo(0); tone(220, .05); return }
