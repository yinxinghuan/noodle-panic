...
- 状态模型为 `start | playing | over`，结局原因为 `sealed | noodle | timeout`。玩家必须点亮 12 颗不同净化球才会立即以 `sealed` 成功；时间耗尽但未满 12 颗为 `timeout`，零操作不会成功。只有 `sealed` 会从 DNA 基因池生成下一株；污染与超时均保留当前菌株以便重试。hook 用 `requestAnimationFrame` 根据剩余秒数将各菌株的基础移动间隔缩短至 80% 与 62%。
- `strains.ts` 的 DNA 包含色素、形体、动态、轨迹、分裂：每个基因从 4 个离散值随机抽取，杆菌/链菌/弧菌/孢菌分别占用 1–4 个危险格。渲染层依据动态 DNA 输出错峰的抖动、蠕动、呼吸或停顿式旋转；具有分裂基因的菌株每 1.8 秒进行 650 ms 的母体收缩与实体芽体长出，并同步扩展危险格。
